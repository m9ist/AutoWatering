...
столько-то времени». Переводной коэффициент — `POMP_FLOW_ML_PER_SEC`
в [src/Pomp.h](src/Pomp.h): практический расход помпы в мл/с. По нему
считается длительность автополива и `Expected ml` в отчёте.

Текущее значение — **7.7 мл/с**. Оно унаследовано из прежней константы
`practicalSpeedMlInMs = 0.0077` (мл/мс); когда и как его мерили,
...
5. Прописать новое значение в `POMP_FLOW_ML_PER_SEC` и перепрошить Mega.
...
переделки гидравлики калибровку надо повторить. Расход резервной помпы
отдельно не измерялся: код считает её такой же, как активную.
