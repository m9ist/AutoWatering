...
- Оживление залипших после простоя клапанов: сухой прогон «дожим + щелчки» (`/wakeup`, либо кнопка проливки растения при выключенном тумблере помпы)
...
[Arduino Mega2560] ── помпы, клапаны, датчики, дисплей, RTC, кнопки, SD-лог
```
...
| DC-DC преобразователь | LM2596S | 2 шт. Питание от внешнего БП 20V: один понижает до 5V (клапаны, электроника), второй до 12V (помпы) |
...
| Помпа | R385 + драйвер MX1508 (чип L298N) | 2 шт. Перистальтические, активная выбирается из прошивки (ADR-0002). [Документация L298N](https://robotchip.ru/obzor-drayvera-motora-na-l298n/) |
...
| `/state` | Последний стейт из retained `aw/state` (влажность, температура, свободная память, активная помпа) |
...
| `/wakeup plantX [AxBms CxDms]` | Расходить залипший клапан X: щелчки всухую, помпа не включается. Без аргументов — `3x2000ms + 7x300ms`. Пример: `/wakeup plant3 5x1500 10x400` |
...
| ESP → Arduino | `esp_pump` | Переключить активную помпу на другую |
...
- Резерв помпы проверяется вероятностно (10% поливов уходят на неактивную помпу); при мёртвом резерве эти поливы уходят в никуда, вердикт выносит человек по `/state` (ADR-0002)
