...
Объём полива человек задаёт в миллилитрах, а помпа умеет только «крутиться
столько-то времени». Переводной коэффициент — практический расход помпы
в мл/с; по нему считается длительность автополива и `Expected ml` в отчёте.

Помпы физически разные, поэтому коэффициент у каждой свой
([src/Pomp.h](src/Pomp.h)):

| Помпа | Константа | Расход |
|-------|-----------|--------|
| 1 (main) | `POMP_FLOW_ML_PER_SEC_1` | 2.1 мл/с |
| 2 (spare) | `POMP_FLOW_ML_PER_SEC_2` | 3.2 мл/с |

Код берёт нужную через `pumpFlowMlPerSec(pumpIdx)` — в том числе на пробе
резерва, когда полив уходит на неактивную помпу.

Порядок калибровки:

1. Включить тумблер мотора (иначе кнопка запускает оживление клапана, а не
   полив), подставить под шланг растения мерный стакан.
2. Подержать кнопку проливки этого растения — льётся, пока кнопка нажата.
3. Взять из отчёта (экран и Telegram) `Expected ml` и записать реально
   налитый объём по стакану.
4. Новое значение = текущее × (налито по стакану / `Expected ml`). Несколько
   проливов разной длительности дают более честное среднее: короткие
   искажает разгон помпы.
5. Прописать новое значение в константу **той помпы, которая лила** (номер
   есть в том же отчёте, строка `Pump N`) и перепрошить Mega.

Кнопка на корпусе всегда льёт активной помпой — чтобы откалибровать вторую,
переключись на неё командой `/pump` и не забудь вернуть обратно.

Делить объём на `Duration` из отчёта нельзя: `Duration` — это весь полив
целиком, включая паузы по 200 мс на открытие и закрытие клапана, когда помпа
ещё (уже) не качает. `Expected ml` считается от окна работы самой помпы, так
что отношение «налито / Expected» этой погрешности не содержит.

`Real ml` в том же отчёте — показания расходомера YF-S401, они меряют то же
самое независимо, но сами нуждаются в проверке стаканом; ориентир при
калибровке — стакан, а не они.

Значение зависит от помпы, её скорости и высоты подъёма шланга — после любой
переделки гидравлики калибровку надо повторить, по каждой помпе отдельно.
